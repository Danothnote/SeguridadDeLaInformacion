...
abc2svg.Abc = function (user) {
    ...
    function y_set(st, up, x, w, y) {
        var p_staff = staff_tb[st], i = (x / realwidth * YSTEP) | 0, j = ((x + w) / realwidth * YSTEP) | 0
        if (i < 0)
            i = 0
        if (j >= YSTEP) {
            j = YSTEP - 1
    ...
    function d_arp(de) {
        ...
        if (s.type == C.NOTE) {
            for (m = 0; m <= s.nhd; m++) {
                if (s.notes[m].acc) { dx = 5 + s.notes[m].shac } else {
                    dx = 6 - s.notes[m].shhd
                    switch (s.head) {
    ...
    function d_cresc(de) {
        ...
        if (de.defl.noen) {
            dx = de.x - x
            if (dx < 20) { x = de.x - 20 - 3; dx = 20 }
        } else {
            x2 = s2.x; de_next = a_de[de.ix + 1]
    ...
    function d_pf(de) {
        var dd2, x2, x, up, s = de.s, dd = de.dd, de_prev; if (de.ldst)
            return
        if (de.start) {
            d_cresc(de)
            return
    ...
    function d_trill(de) {
        ...
        if (up) {
            ...
            y -= dd.h; tmp = staff_tb[s.st].botbar - 2
            if (y > tmp)
                y = tmp
        }
    ...
    function d_upstaff(de) {
        ...
        if (dd.func == 4) { up = 0 } else if (s.pos) {
            switch (s.pos.orn) {
                case C.SL_ABOVE: up = 1
                    break
        ...
        switch (dd.glyph) {
            case "accent": case "roll": if (!up || (up < 0 && (s.multi < 0 || (!s.multi && s.stem > 0)))) {
                yc = y_get(s.st, false, s.x - dd.wl, w) - 2
                if (yc > staffb)
                    yc = staffb; yc -= dd.h; y_set(s.st, false, s.x, 0, yc); inv = true; s.ymn = yc
            } else {
    ...
    function deco_def(nm) {
        ...
        if (c == '(' || (c == ')' && dd.name.indexOf('(') < 0)) {
            dd.str = null; name2 = dd.name.slice(0, -1) + (c == '(' ? ')' : '('); dd2 = dd_tb[name2]
            if (dd2) { if (c == '(') { dd.dd_en = dd2; dd2.dd_st = dd } else { dd.dd_st = dd2; dd2.dd_en = dd } } else {
                dd2 = deco_def(name2)
                if (!dd2)
                    return
            }
    ...
    function deco_cnv(a_dcn, s, prev) {
        ...
        for (i = 0; i < nd; i++) {
            ...
            switch (dd.func) {
                case 0: if (s.type == C.BAR && dd.name == "dot") {
                    s.bar_dotted = true
                    break
                }
    ...
    function deco_width(s) {
        var dd, i, wl = 0, a_dd = s.a_dd, nd = a_dd.length
        for (i = 0; i < nd; i++) {
            dd = a_dd[i]
            switch (dd.func) {
                case 1: if (wl < 7)
    ...
    function draw_all_deco() {
        ...
        while (1) {
            ...
            if (de.m != undefined) { note = s.notes[de.m]; x += note.shhd * stv_g.scale } else if (f_staff[dd.func] && !cfmt.dynalign && ((de.up && st > 0) || (!de.up && st < nstaff))) {
                if (de.up)
                    ym = ymid[--st]
                else
    ...
    function draw_deco_near() {
        ...
        function create_dh(s, m) {
            var f, str, de, uf, k, dcn, dd, note = s.notes[m], nd = note.a_dcn.length
            for (k = 0; k < nd; k++) {
                dcn = note.a_dcn[k]; dd = dd_tb[dcn]
                if (!dd) {
                    dd = deco_def(dcn)
                    if (!dd)
                        continue
        ...
        function ll_deco() {
            ...
            for (i = 0; i < n_de; i++) {
                ...
                if (j == n_de) {
                    de2 = { s: de.s, st: de.st, dd: dd2, ix: a_de.length - 1, x: realwidth - 6, y: de.s.y, lden: true, defl: { noen: true } }
                    if (de2.x < s.x + 10)
                        de2.x = s.x + 10
                    if (de.m != undefined)
    ...
    function draw_deco_staff() {
        var s, first_gchord, p_voice, x, y, w, i, v, de, dd, gch, gch2, ix, top, bot, minmax = new Array(nstaff + 1), nd = a_de.length
        function draw_repbra(p_voice) {
            var s, s1, y, y2, i, p, w, wh, first_repeat; y = staff_tb[p_voice.st].topbar + 25
            for (s = p_voice.sym; s; s = s.next) {
                ...
                if (y < y2)
                    y = y2
                if (s1.text) {
                    wh = strwh(s1.text); y2 = y_get(p_voice.st, true, s1.x + 4, wh[0]); y2 += wh[1]
                    if (y < y2)
                        y = y2
                }
                if (s.rbstart)
            ...
            for (; s; s = s.next) {
                ...
                if (!s.next && !s.rbstop && !p_voice.bar_start) {
                    p_voice.bar_start = clone(s); p_voice.bar_start.type = C.BAR; p_voice.bar_start.bar_type = ""
        ...
        if (first_gchord) {
            for (i = 0; i <= nstaff; i++) {
                bot = staff_tb[i].botbar; if (minmax[i].ymin > bot - 4)
                    minmax[i].ymin = bot - 4
        ...
        if (bar_num > 1) {
            if (cfmt.measurenb == 0) {
                ...
                if (y < staff_tb[st].topbar + 14)
                    y = staff_tb[st].topbar + 14; xy_str(0, y, bar_num.toString()); y_set(st, true, 0, 20, y + gene.curfont.size + 2)
            } else if (bar_num % cfmt.measurenb == 0) {
                for (; ; s = s.ts_next) {
                    switch (s.type) { case C.TIMESIG: case C.CLEF: case C.KEYSIG: case C.FMTCHG: case C.STBRK: continue }
                    break
                }
    ...
    function draw_notempo(s, x, y, dur, sc) {
        ...
        if (dur < C.BLEN) {
            if (nflags <= 0) { out_stem(-posx, posy, 21) } else {
    ...
    function write_tempo(s, x, y) {
        var j, dx, bx, bh, sc = .7 * gene.curfont.size / 12.0; set_font("tempo")
        if (gene.curfont.box) {
    ...
    function draw_partempo(st, top) {
        ...
        if (some_tempo) {
            set_sscale(-1); set_font("tempo"); ht = gene.curfont.size + 8; y = 2 - ht; h = y - ht
            if (dosh != 0)
                ht *= 2
            if (top < ymin + ht)
                dy = ymin + ht - top
    ...
    Abc.prototype.calculate_beam = function (bm, s1) {
        ...
        for (s = s1; ; s = s.next) {
            ...
            if (s.stem >= 0) {
                ...
                if (s.notes[s.nhd].pit > p_max) { p_max = s.notes[s.nhd].pit; s_closest = s }
            } else {
        ...
        for (s = s1; ; s = s.next) {
            switch (s.type) {
                case C.NOTE: s.ys = a * s.xs + b - staff_tb[s.st].y
                    if (s.stem > 0) {
                        s.ymx = s.ys + 2.5
                        if (s.ts_prev && s.ts_prev.stem > 0 && s.ts_prev.st == s.st && s.ts_prev.ymn < s.ymx && s.ts_prev.x == s.x && s.notes[0].shhd == 0) { s.ts_prev.x -= 3; s.ts_prev.xs -= 3 }
                    } else { s.ymn = s.ys - 2.5 }
                    break
...
<text x="X" y="Y" text-anchor="middle">A</text>\n', x, y + 12, m_gl(meter.top))
    ...
    function draw_acc(x, y, acc, micro_n, micro_d) {
        if (micro_n) {
            if (micro_n == micro_d) { acc = acc == -1 ? -2 : 2 } else if (micro_n * 2 != micro_d) {
    ...
    Abc.prototype.draw_hl = function (x, s, hltype) {
        var i, j, n, hla = [], st = s.st, p_staff = staff_tb[st]
        if (!p_staff.hll)
            return
        for (i = 0; i <= s.nhd; i++) {
    ...
    Abc.prototype.draw_keysig = function (x, s) {
        ...
        if (!s.k_a_acc) {
            ...
            var acc, last_acc = s.k_a_acc[0].acc, last_shift = 100, s2 = { st: st, nhd: 0, notes: [{}] }
            for (i = 0; i < s.k_a_acc.length; i++) {
                acc = s.k_a_acc[i]; shift = (s.k_y_clef
                    + acc.pit - 18) * 3
        ...
        if (s.rep_nb) {
            ...
            if (s.rep_nb < 0) { xygl(x, yb, "srep") } else {
                ...
                if (s.rep_nb > 2 && s.v == cur_sy.top_voice) {
                    set_font("annotation"); if (gene.curfont.box) { gene.curfont.box = false; bx = true }
    ...
    function draw_gracenotes(s) {
        var yy, x0, y0, x1, y1, x2, y2, x3, y3, bet1, bet2, slur, dy1, dy2, g, last, note, bm = {}
        ...
        if (s.sappo) {
            g = s.extra
            if (!g.next) { x1 = 9; y1 = g.stem > 0 ? 5 : -5 } else {
        ...
        if (s.p_v.key.k_bagpipe || !cfmt.graceslurs || slur || s.tie_s || !s.next || s.next.type != C.NOTE)
            return
        last = g
        if (((g.stem >= 0 || s.multi < 0) && g.notes[0].pit <= 28) || g.notes[0].pit < 16) {
            yy = 127
    ...
    function draw_basic_note(x, s, m, y_tb) {
        var i, p, yy, dotx, doty, inv, old_color = false, note = s.notes[m], staffb = staff_tb[s.st].y, y = 3 * (note.pit - 18), shhd = note.shhd * stv_g.scale, x_note = x + shhd, y_note = y + staffb
    ...
    function draw_note(s, fl) {
        ...
        if (s.grace) { hltype = "ghl"; x_hl += 1.5 } else {
            hltype = "hl"
            switch (s.head) {
                case C.OVALBARS: case C.OVAL: x_hl -= .3; hltype = "hl1"
                    break
        ...
        if (!s.stemless) {
            ...
            } else { out_stem(x, y, slen, s.grace, nflags, cfmt.straightflags) }
    ...
    function draw_slur(path, not1, sl) {
        var i, k, g, x1, y1, x2, y2, height, addy, a, y, z, h, dx, dy, ty = sl.ty, dir = (ty & 0x07) == C.SL_ABOVE ? 1 : -1, n = path.length, i1 = 0, i2 = n - 1, k1 = path[0], k2 = path[i2]
        ...
        if (not1) {
            ...
            if (k1.type == C.NOTE) {
                if (dir > 0) {
                    if (k1.stem > 0) {
                        x1 += 5
                        if (k1.beam_end && k1.nflags >= -1 && !k1.in_tuplet) { if (k1.nflags > 0) { x1 += 2; y1 = k1.ys - 3 } else { y1 = k1.ys - 6 } }
                    }
                } else {
                    if (k1.stem < 0) {
                        x1 -= 1
    ...
    function draw_slurs(s, last) {
        ...
        function draw_sls(s, sl, snote) {
            var k, v, i, dir, path = [], enote = sl.note, s2 = enote.s
    ...
    function draw_tuplet(s1, tp) {
        ...
        if (s1 == s2 || tp.f[1] == 2) { nb_only = true } else if (tp.f[1] == 1) { nb_only = true; draw_slur([s1, s2], null, { ty: dir }) } else {
        ...
        if (nb_only) {
            ...
            if (dir == C.SL_ABOVE) {
                ym += 10
                if (s3.ymx < ym)
                    s3.ymx = ym; y_set(upstaff, true, xm - 3, 6, ym)
            } else {
                if (s3.ymn > ym)
        ...
        if (dir == C.SL_ABOVE) {
            if (s1.st == s2.st) { y1 = y2 = staff_tb[upstaff].topbar + 4 } else { y1 = s1.ymx; y2 = s2.ymx }
            x1 = s1.x - 4; if (s1.st == upstaff) {
                for (s3 = s1; !s3.dur; s3 = s3.next); ym = y_get(upstaff, 1, s3.x - 4, 8)
                if (ym > y1)
                    y1 = ym
                if (s1.stem > 0)
                    x1 += 3
            ...
            if (s2.st == upstaff) {
                for (s3 = s2; !s3.dur; s3 = s3.prev); ym = y_get(upstaff, 1, s3.x - 4, 8)
                if (ym > y2)
                    y2 = ym
            }
    ...
    function draw_note_ties(not1, job) {
        var m, x1, x2, s, y, h, not2 = not1.tie_n, p = job == 2 ? not1.pit : not2.pit, dir = (not1.tie_ty & 0x07) == C.SL_ABOVE ? 1 : -1, s1 = not1.s, st = s1.st, s2 = not2.s, x2 = s2.x, sh = not1.shhd
        for (m = 0; m < s1.nhd; m++)
            if (s1.notes[m] == not1)
                break
        if (dir > 0) {
        ...
        switch (job) {
            case 0: p = (not1.pit & 1) ? not1.pit : not2.pit
                break
            case 3: dir = -dir
            case 1: x1 = s1.x
                if (x1 > x2 - 20)
    ...
    function draw_ties(k1, k2, job) {
        var k3, i, j, not1, not3, time, pit, pit2, mhead3 = [], nh1 = k1.nhd
        if (job == 2) {
            for (i = 0; i <= nh1; i++) {
                not1 = k1.notes[i]
                if (not1.tie_ty) {
    ...
    function draw_all_ties(p_voice) {
        ...
        while (1) {
            ...
            if (clef_chg || s1.st != s2.st) {
                clef_chg = false; dx = (s2.x - s1.x) * .4; x = s2.x; s2.x -= dx
                if (s2.x > s1.x + 32.)
    ...
    function draw_sym_near() {
        var p_voice, p_st, s, v, st, y, g, w, i, st, dx, top, bot, output_sav = output; output = ""
        for (v = 0; v < voice_tb.length; v++) {
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            if (s.invis)
                continue
            switch (s.type) {
                case C.GRACE: for (g = s.extra; g; g = g.next) { y_set(s.st, true, g.x - 2, 4, g.ymx + 1); y_set(s.st, false, g.x - 2, 4, g.ymn - 1) }
                    continue
            ...
            if (s.stem > 0) {
                ...
                y_set(s.st, true, s.x + dx, w, s.ymx); y_set(s.st, false, s.x - s.wl, s.wl + s.wr, s.ymn)
            } else {
    ...
    function set_staff() {
        ...
        y = 0
        if (st > nstaff) { st--; p_staff = staff_tb[st] } else {
            p_staff = staff_tb[st]
            for (i = 0; i < YSTEP; i++) {
                val = p_staff.top[i]
                if (y < val)
    ...
    function draw_systems(indent) {
        var s, s2, st, x, x2, res, sy, staves_bar, bar_force, xstaff = [], bar_bot = [], bar_height = [], ba = [], sb = [], thb = []
        function bar_set() {
        ...
        function draw_staff(st, x1, x2) {
            var w, ws, i, dy, ty, y = 0, ln = "", stafflines = cur_sy.staves[st].stafflines, l = stafflines.length
            if (!/[\[|]/.test(stafflines))
                return
            ...
            if (!cache && w > get_lwidth() - 10) {
                cache = { st_l: stafflines, st_ws: ws | 0 }
        ...
        function draw_bar(s, bot, h) {
            ...
            for (i = bar_type.length; --i >= 0;) {
                switch (bar_type[i]) {
                    case "|": if (s.bar_dotted) { set_sscale(-1); w = (5 * p_staff.staffscale).toFixed(1); out_XYAB('<path class="bW" stroke-dasharray="A,A" d="MX Yv-G"/>\n', x, bot, w, h) } else if (s.color) { out_XYAB('<path class="bW" d="MX Yv-F"/>\n', x, bot, h) } else { sb.push(new Float32Array([x, bot, h])) }
                        break
        ...
        function out_bars() {
            ...
            set_font("annotation"); if (gene.curfont.box) {
                gene.curfont.box = false
                bx = true
            }
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            switch (s.type) {
                case C.STAVES: staves_bar = s.ts_prev.type == C.BAR ? s.ts_prev.x : 0
                    if (!staves_bar) {
                        for (s2 = s.ts_next; s2; s2 = s2.ts_next) {
                            if (s2.time != s.time)
                                break
                            switch (s2.type) {
                                case C.BAR: case C.CLEF: case C.KEY: case C.METER: staves_bar = s2.x
                                    continue
                            }
                            break
                        }
    ...
    Abc.prototype.draw_symbols = function (p_voice) {
        var bm = {}, s, g, x, y, st; for (s = p_voice.sym; s; s = s.next) {
            if (s.invis) {
                switch (s.type) {
...
N "Notes: "\n\
Z "Transcription: "\n\
H "History: "', infospace: 0, keywarn: true, leftmargin: 1.4 * CM, lineskipfac: 1.1, linewarn: true, maxshrink: .65, maxstaffsep: 2000, maxsysstaffsep: 2000, measurefont: { name: "serif", style: "italic", size: 10 }, measurenb: -1, musicspace: 6, partsfont: { name: "serif", size: 15 }, parskipfac: .4, partsspace: 8, pagewidth: 13 * CM, printmargin: 0, rightmargin: 0.5 * CM, rbdbstop: true, rbmax: 4, rbmin: 2, repeatfont: { name: "serif", size: 13 }, scale: 1, slurheight: 1.0, staffsep: 46, stemheight: 21, stretchlast: .25, stretchstaff: true, subtitlefont: { name: "serif", size: 16 }, subtitlespace: 3, sysstaffsep: 34, tempofont: { name: "serif", weight: "bold", size: 12 }, textfont: { name: "serif", size: 16 }, textspace: 14, titlefont: { name: "serif", weight: 'bold', size: 20 }, titlespace: 6, titletrim: true, topspace: 22, tuplets: [0, 0, 0, 0], vocalfont: { name: "serif", weight: "bold", size: 13 }, vocalspace: 10, voicefont: { name: "serif", weight: "bold", size: 13 }, writefields: "CMOPQsTWw", wordsfont: { name: "serif", size: 16 }, wordsspace: 5
    }
    ...
    Abc.prototype.set_format = function (cmd, param) {
        ...
        switch (cmd) {
            case "aligncomposer": case "barsperstaff": case "infoline": case "measurenb": case "rbmax": case "rbmin": case "shiftunison": v = parseInt(param)
    ...
    function use_font(font) {
        if (!font.used) {
            font.used = true; if (font.fid == undefined) {
                font.fid = font_tb.length; font_tb.push(font)
                if (!font.swfac)
                    set_font_fac(font)
            }
    ...
    function tosvg(in_fname, file, bol, eof) {
        ...
        for (; bol < eof; bol = parse.eol + 1) {
            ...
            } else if (line0 == 'I' && line1 == ':') { pscom = true }
            if (pscom) {
                pscom = false; bol += 2
    ...
    function identify_note(s, dur_o) {
        ...
        if (flags >= 0) { head = C.FULL } else switch (flags) {
            default: error(1, s, "Note too long"); flags = -4
    ...
    function set_acc_shft() {
        var s, s2, st, i, acc, st, t, dx_head; s = tsfirst
        while (s) {
    ...
    function lktsym(s, next) {
        if (next) {
            s.ts_next = next; s.ts_prev = next.ts_prev
            if (s.ts_prev)
                s.ts_prev.ts_next = s; next.ts_prev = s
        } else { s.ts_next = s.ts_prev = null }
    }
    function unlksym(s) {
        if (s.next)
            s.next.prev = s.prev
        if (s.prev)
            ...
            s.p_v.sym = s.next
        if (s.ts_next) {
            if (s.seqst) { if (s.ts_next.seqst) { s.ts_next.shrink += s.shrink; s.ts_next.space += s.space } else { s.ts_next.seqst = true; s.ts_next.shrink = s.shrink; s.ts_next.space = s.space } }
    ...
    function set_graceoffs(s) {
        var next, m, dx, x, gspleft = cfmt.gracespace[0], gspinside = cfmt.gracespace[1], gspright = cfmt.gracespace[2], g = s.extra; if (s.prev && s.prev.type == C.BAR)
    ...
    Abc.prototype.set_width = function (s) {
        var s2, i, m, xx, w, wlnote, wlw, acc, bar_type, meter, last_acc, n1, n2, esp, tmp
        switch (s.type) {
            case C.NOTE: case C.REST: s.wr = wlnote = hw_tb[s.head]
                if (s.xmx > 0)
                ...
                if (s2) {
                    switch (s2.type) {
                        case C.NOTE: if (s2.stem > 0 && s.stem < 0) {
                            if (wlw < 7)
                                wlw = 7
                        }
            ...
            case C.CLEF: if (s.invis) {
                ...
                return
            case C.KEY: if ((!s.k_a_acc && !s.k_sf && !s.k_old_sf) || s.k_none || s.k_play) {
                s.wl = s.wr = 0
                return
            }
                s.wl = 3; esp = 4
    ...
    function time2space(s, len) {
        ...
        space = space_tb[i]
        if (l != 0) {
            if (l < 0) { space = space_tb[0] * len / (C.BLEN / 16 / 8) } else {
                if (i >= 9)
                    i = 8
    ...
    function set_space(s) {
        var s2, space, prev_time = s.ts_prev.time, len = s.time - prev_time
        if (len == 0) {
    ...
    function set_sp_tup(s, s_et) {
        var s2, tim = s.time, endtime = s_et.time + s_et.dur, ttim = endtime - tim, space = time2space(s, ttim / s.tp[0].q) * s.tp[0].q / ttim
        ...
        while (1) {
            if (s.seqst) { s.space = (s.time - tim) * space; tim = s.time }
            if (s == s_et)
                break
            s = s.ts_next
        }
    ...
    function add_end_bar(s) {
        var bar = { type: C.BAR, bar_type: "|", fname: s.fname, istart: s.istart, iend: s.iend, v: s.v, p_v: s.p_v, st: s.st, dur: 0, seqst: true, invis: true, time: s.time + s.dur, nhd: 0, notes: [{ pit: s.notes[0].pit }], wl: 0, wr: 0, prev: s, ts_prev: s, shrink: s.wr + 3 }
        s.next = s.ts_next = bar
        return bar
    }
    function set_allsymwidth() {
        var maxx, new_val, s_tupc, s_tupn, st, s_chs, s = tsfirst, s2 = s, xa = 0, xl = [], wr = [], ntup = 0
        while (1) {
            maxx = xa
            do {
                if (s.a_gch && !s_chs)
    ...
    function set_repeat(s) {
        ...
        if (n < 0) {
            ...
            for (j = k; --j >= 0;) {
                ...
                if (s.seqst)
                    s.space = set_space(s); s.head = C.SQUARE; for (s = s2; s; s = s.ts_next) {
        ...
        if (n == 2) {
            ...
            if (s2.seqst)
                s2.space = set_space(s2); s3 = s2.next; for (s2 = s3.ts_next; ; s2 = s2.ts_next) {
                    if (s2.st != st)
                        continue
                    if (s2.v == v && s2.type == C.BAR)
                        break
    ...
    function set_nl(s) {
        ...
        if (cfmt.keywarn || cfmt.timewarn) {
            for (s2 = s.ts_next; s2; s2 = s2.ts_next) {
                switch (s2.type) {
                    case C.BAR: case C.CLEF: continue
                    ...
                    case C.METER: if (s2.type == C.METER && !cfmt.timewarn)
                        ...
                        while (1) {
                            s2 = s2.ts_prev
                            if (s2.v == s.v) {
                                s.next = s2.next; s.prev = s2; s.next.prev = s; s2.next = s
                                break
                            }
                        }
        ...
        switch (s.type) {
            case C.CLEF: case C.BAR: case C.STAVES: break
        ...
        for (; ; s = s.ts_next) {
            ...
            if (done < 0)
                break
            switch (s.type) {
                case C.STAVES: if (s.ts_prev && s.ts_prev.type == C.BAR)
                    break
                    while (s.ts_next) {
    ...
    function get_width(s, last) {
        var shrink, space, w = 0, wmx = 0, sp_fac = (1 - cfmt.maxshrink)
        do {
            if (s.seqst) {
                shrink = s.shrink
                wmx += shrink
    ...
    function set_lines(s, last, lwidth, indent) {
        var first, s2, s3, x, xmin, xmid, xmax, wwidth, shrink, space, nlines, cut_here, ws = get_width(s, last)
        ...
        while (1) {
            ...
            s2 = first = s; xmin = s.x - s.shrink - indent; xmax = xmin + lwidth; xmid = xmin + wwidth / nlines; xmin += wwidth / nlines * cfmt.breaklimit; for (s = s.ts_next; s != last; s = s.ts_next) {
                if (!s.x)
                    continue
                if (s.type == C.BAR)
                    s2 = s
            ...
            if (!cut_here) {
                var beam = 0, bar_time = s2.time; xmax -= 8; s = s2; s3 = null
                for (; s != last; s = s.ts_next) {
                    if (s.beam_st)
                        beam++
    ...
    function cut_tune(lwidth, indent) {
        ...
        if (cfmt.barsperstaff) {
            i = cfmt.barsperstaff; for (s2 = s; s2; s2 = s2.ts_next) {
    ...
    function set_ottava() {
        var s, st, delta, note, g, m = nstaff + 1, staff_d = new Int16Array(new Array(m * 2)), staff_noo = new Int8Array(new Array(m))
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            if (s.ottava != undefined)
                while (s.ottava.length) {
                    if (s.ottava[0]) { if (staff_noo[st]++ == 0) { ottava_add(s, s.ottava[0], true); staff_d[st] = -s.ottava[0] * 7 } } else { if (--staff_noo[st] == 0) { ottava_add(s, -staff_d[st] / 7); staff_d[st] = 0 } }
                    s.ottava.shift()
                }
            switch (s.type) {
    ...
    Abc.prototype.set_pitch = function (last_s) {
        var s, s2, g, st, delta, pitch, note, dur = C.BLEN, m = nstaff + 1, staff_delta = new Int16Array(new Array(m * 2)), sy = cur_sy
        for (st = 0; st <= nstaff; st++) {
    ...
    function new_sym(type, p_voice, last_s) {
        var s = { type: type, fname: last_s.fname, v: p_voice.v, p_v: p_voice, st: p_voice.st, time: last_s.time }
        if (p_voice.last_sym) {
    ...
    function init_music_line() {
        ...
        for (v = 0; v < nv; v++) {
            ...
            if (s.next)
                ...
                s.ts_prev = null
            if (!s.ts_prev) { tsfirst = s; s.seqst = true } else {
                s.ts_prev.ts_next = s
                delete s.seqst
            }
            if (last_s) {
        ...
        for (v = 0; v < nv; v++) {
            ...
            if (last_s && last_s.v == v && last_s.type == C.KEY) {
                p_voice.last_sym = last_s; last_s.k_old_sf = last_s.k_sf; last_s = last_s.ts_next
                continue
            }
            s2 = p_voice.key
            if (s2.k_sf || s2.k_a_acc) {
                s = new_sym(C.KEY, p_voice, last_s); s.k_sf = s2.k_sf; s.k_mode = s2.k_mode; s.k_old_sf = s2.k_sf; s.k_a_acc = s2.k_a_acc; s.istart = s2.istart; s.iend = s2.iend
                if (s2.k_bagpipe) {
        ...
        while (1) {
            s2 = s; shrmx = 0
            do {
                self.set_width(s); shr = s.wl
                if (s.prev)
                    shr += s.prev.wr
                if (shr > shrmx)
                    shrmx = shr; s = s.ts_next
            } while (s != last_s && !s.seqst); s2.shrink = shrmx; s2.space = 0
            if (s == last_s)
                break
        }
        shr = 0
        do {
            self.set_width(s)
            if (shr < s.wl)
                shr = s.wl; s = s.ts_next
        } while (s && !s.seqst); last_s.shrink = s2.wr + shr
        last_s.space = 0
    }
            ...
            switch (s.type) {
                ...
                case C.NOTE: case C.REST: res = identify_note(s, s.dur_orig); s.head = res[0]; s.dots = res[1]; s.nflags = res[2]
                    ...
                    if (s.next && s.next.trem2)
                        break
                    if (s.trem2) {
                        s.prev.trem2 = true; s.prev.nflags = --s.nflags; s.prev.head = ++s.head
                        if (s.nflags > 0) { s.nflags += s.ntrem } else {
                            if (s.nflags <= -2) { s.stemless = true; s.prev.stemless = true }
                            s.nflags = s.ntrem
                        }
    ...
    function set_global() {
        ...
        sy = cur_sy; st = sy.nstaff; while (1) {
            sy = sy.next
            if (!sy)
                break
            if (sy.nstaff > st)
        ...
        for (s = sym; s; s = s.next) {
            ...
            if (!s.stem && (s.stem = s.multi) == 0) {
                ...
                if (beam) { s.stem = laststem } else if (s.beam_st && !s.beam_end) {
                    ...
                    if (g.beam_end) {
                        if ((pu + pd) / 2 < mid_p) { s.stem = 1 } else if ((pu + pd) / 2 > mid_p) { s.stem = -1 } else {
                            if (cfmt.bstemdown)
                                s.stem = -1
                        }
                    }
    ...
    function set_overlap() {
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            while (1) {
                ...
                switch (dp) {
                    case 0: if (s1.notes[i1].acc != s2.notes[i2].acc && !s1.notes[i1].acc && !s2.notes[i2].acc) {
                        t = -1
                        break
                    }
                        if (s2.notes[i2].acc) {
                    ...
                    case -2: if (s1.dots && s2.dots && !(s1.notes[i1].pit & 1)) {
                        s1.dot_low = false; s2.dot_low = false
                        break
                    }
                        break
                }
    ...
    Abc.prototype.set_stems = function () {
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            if (s.type != C.NOTE) {
                ...
                ymin = ymax = s.mid
                for (g = s.extra; g; g = g.next) {
                    res = identify_note(s, g.dur); g.head = res[0]; g.dots = res[1]; g.nflags = res[2]; slen = GSTEM
                    if (g.nflags > 1)
    ...
    function check_bar(s) {
        ...
        while (1) {
            switch (s2.type) {
                case C.BAR: case C.GRACE: case C.NOTE: case C.REST: break
                default: s2 = s2.next
                    if (!s2)
                        return
            }
        ...
        if (s2.type == C.BAR) { s_bs = s2 } else {
            ...
            if (s_bs == tsnext || s2.ts_prev.type != C.BAR) {
                if (s2.seqst) { s_bs.seqst = true } else {
                    s2.seqst = true
        ...
        if (i < bar_type.length) {
            ...
            while (bar_type[i] == ':')
                i--; s_bs.bar_type = '[|' + bar_type.slice(i + 1)
        } else { i = (bar_type.length / 2) | 0; s.bar_type = bar_type.slice(0, i) + '|]'; s_bs.bar_type = '[|' + bar_type.slice(i) }
        return true
    }
    ...
    function block_gen(s) {
        switch (s.subtype) {
            case "leftmargin": case "rightmargin": case "pagescale": case "pagewidth": case "scale": case "staffwidth": svg_flush(); self.set_format(s.subtype, s.param)
                break
    ...
    function set_piece() {
        var s, last, p_voice, st, v, nst, nv, tmp, non_empty = [], non_empty_gl = [], sy = cur_sy
        ...
        for (st = 0; st <= nst; st++)
            reset_staff(st); for (s = tsfirst; s; s = s.ts_next) {
                if (s.nl) { break }
                if (!s.ts_next)
                    last = s
                switch (s.type) {
                    case C.STAVES: set_brace(); sy.st_print = new Uint8Array(non_empty); sy = s.sy; nst = sy.nstaff
                        if (nstaff < nst) {
            ...
            for (v = 0; v < nv; v++) {
                ...
                if (p_voice.sym && p_voice.sym.time <= tsnext.time) {
                    for (s = tsnext.ts_prev; s; s = s.ts_prev) {
                        if (s.v == v) {
                            p_voice.s_next = s.next; s.next = null; if (check_bar(s)) { tmp = s.wl; self.set_width(s); s.shrink += s.wl - tmp }
                            break
                        }
                    }
                    if (s)
        ...
        if (last.type != C.BAR) {
            s = add_end_bar(last); s.space = set_space(s)
            if (s.space < s.shrink && last.type != C.KEY)
                s.space = s.shrink
        }
    }
    Abc.prototype.set_sym_glue = function (width) {
        var s, g, ll, some_grace, spf, xmin = 0, xx = 0, x = 0, xs = 0, xse = 0
        for (s = tsfirst; s; s = s.ts_next) {
        ...
        if (xmin >= width) {
            ...
            if (xx == xse)
                xx += 5
            for (var cnt = 4; --cnt >= 0;) {
                spf = (width - xs - xse) / (xx - xse); xx = 0; xse = 0; x = 0
                for (s = tsfirst; s; s = s.ts_next) {
    ...
    function gen_init() {
        ...
        for (; s; s = s.ts_next) {
            ...
            switch (s.type) {
                case C.NOTE: case C.REST: case C.MREST: set_page()
                    return
                default: continue
    ...
    Abc.prototype.output_music = function () {
        ...
        spf_last = 1.2
        while (1) {
            set_piece(); self.set_sym_glue(lwidth - indent)
            if (realwidth != 0) {
                if (indent != 0)
    ...
    function get_interval(param, score) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (!note) {
                ...
                pit[i] = 23
            } else { pit[i] = abc2svg.pab40(note.pit, note.acc) }
        }
        return pit[1] - pit[0]
    }
    ...
    function new_key(param) {
        var i, clef, key_end, c, tmp, sf = "FCGDAEB".indexOf(param[0]) - 1, mode = 0, s = { type: C.KEY, dur: 0 }
        function set_k_acc(s, sf) {
            var i, j, n, nacc, p_acc, accs = [], pits = [], m_n = [], m_d = []
            ...
            for (i = 0; i < nacc; i++) {
                ...
                if (m_n[i]) { p_acc.micro_n = m_n[i]; p_acc.micro_d = m_d[i] } else {
        ...
        if (sf < -1) {
            switch (param[0]) {
                case 'H': switch (param[1]) {
                    case 'P': case 'p': s.k_bagpipe = param[1]; sf = param[1] == 'P' ? 0 : 2; i++
                        break
        ...
        if (!key_end) {
            ...
            if (c == '^' || c == '_' || c == '=') {
                ...
                do {
                    ...
                    while (c == ' ')
                        c = param[++tmp.index]
                } while (c == '^' || c == '_' || c == '='); if (!s.k_exp)
                    set_k_acc(s, sf)
                param = param.slice(tmp.index)
            } else if (s.k_exp && param.indexOf("none") == 0) { param = param.replace(/\w+\s*/, '') }
        }
        s.k_sf = sf; if (s.k_a_acc) {
            s.k_map = []
            i = s.k_a_acc.length
            while (--i >= 0) {
                note = s.k_a_acc[i]
        ...
        } else { s.k_map = abc2svg.keys[sf + 7] }
        s.k_mode = mode
        s.k_b40 = [1, 24, 7, 30, 13, 36, 19, 2, 25, 8, 31, 14, 37, 20, 3][sf + 7]
        return [s, info_split(param)]
    }
    function new_meter(p) {
        var s = { type: C.METER, dur: 0, a_meter: [] }, meter = {}, val, v, m1 = 0, m2, i = 0, j, wmeasure, in_parenth; set_ref(s)
    ...
    function new_tempo(text) {
        var i = 0, j, c, nd, tmp, s = { type: C.TEMPO, dur: 0 }
        set_ref(s)
        ...
        if (c == '"') {
            tmp.index++; i = text.indexOf('"', tmp.index + 1)
            if (i < 0) {
                syntax(1, "Unterminated string in Q:")
                return
            }
            ...
            case 'P': if (parse.state == 0)
                ...
                if (curvoice.v != p_voice.v) {
                    ...
                    if (p_voice.last_sym && p_voice.last_sym.type == C.PART)
                        break
                    var voice_sav = curvoice; curvoice = p_voice; sym_link(s); curvoice = voice_sav
                } else { sym_link(s) }
                break
    ...
    function new_bar() {
        ...
        if (bar_type[0] == ':') {
            if (bar_type.length == 1) { bar_type = '|'; s.bar_dotted = true } else {
                s.rbstop = 2
                if (curvoice.tie_s)
        ...
        if (parse.ottava.length) {
            ...
            if (curvoice.cst != curvoice.st) {
                s2 = { type: C.SPACE, fname: parse.fname, istart: parse.bol + line.index, dur: 0, multi: 0, invis: true, width: 1 }
                sym_link(s2)
            }
        ...
        if (s.rbstart && !curvoice.norepbra && curvoice.st > 0 && !(par_sy.staves[curvoice.st - 1].flags & STOP_BAR)) {
            s2 = { type: C.BAR, fname: s.fname, istart: s.istart, iend: s.iend, bar_type: "[", multi: 0, invis: true, text: s.text, rbstart: 2 }
            sym_link(s2); s2.st = curvoice.st
            delete s.text; s.rbstart = 0
        }
    }
    function parse_staves(p) {
        var v, vid, a_vf = [], err = false, flags = 0, brace = 0, bracket = 0, parenth = 0, flags_st = 0, e, a = p.match(/[^[\]|{}()*+\s]+|[^\s]/g)
        if (!a) {
    ...
    function parse_acc_pit(line) {
        ...
        if (acc && acc != 3 && (c >= '1' && c <= '9') || c == '/') {
            nd = parse_dur(line); micro_n = nd[0]; micro_d = nd[1]
            if (micro_d == 1)
    ...
    function set_map(note) {
        var map = maps[curvoice.map], n = note.b40, nn = n.toString()
        if (!map[nn]) {
            nn = 'o' + (n % 40).toString()
            if (!map[nn]) {
                n += 2 - curvoice.ckey.k_b40
    ...
    Abc.prototype.new_note = function (grace, sls) {
        var note, s, in_chord, c, dcn, type, tie_s, not2, i, n, s2, nd, res, num, dur, sl1 = [], line = parse.line, a_dcn_sav = a_dcn; if (!grace && curvoice.tie_s) {
        ...
        switch (c) {
            ...
            default: if (curvoice.uscale)
                ...
                while (1) {
                    if (in_chord) {
                        while (1) {
                            ...
                            switch (type[0]) {
                                ...
                                case '!': if (!a_dcn)
                                    a_dcn = []
                                    if (type.length > 1) { a_dcn.push(type.slice(1, -1)) } else {
                                        dcn = ""
                                        while (1) {
                                            c = line.next_char()
                                            if (!c) {
                    ...
                    i = note.acc
                    if (i) { curvoice.acc[note.pit + 19] = i } else {
            ...
            while (1) {
                ...
                if (c == '.') {
                    switch (line.buffer[line.index + 1]) {
                        case '(': case '-': case '|': c = line.next_char()
                            break
                    }
                }
                idx = c.charCodeAt(0); if (idx >= 128) {
                    syntax(1, errs.not_ascii); line.index++
                    break
                }
                if (!in_mac && maci[idx]) {
                    n = 0
                ...
                switch (type[0]) {
                    ...
                    case '!': if (!a_dcn)
                        a_dcn = []
                        if (type.length > 1) { dcn = type.slice(1, -1) } else {
                            dcn = ""; i = line.index
                            while (1) {
                                c = line.next_char()
                                if (!c)
                            ...
                            if (!c) {
                                line.index = i; syntax(1, "No end of decoration")
                                break
                            }
                        }
    ...
    var strwh = typeof document != "undefined" ? (function (el) {
        ...
        return function (str) {
            ...
            el.style.font = style_font(font).slice(5); str = str.replace(/<|>|&[^&]*?;|&|  /g, function (c) {
                switch (c) {
                    case '<': return "&lt;"
                    case '>': return "&gt;"
                    case '&': return "&amp;"
    ...
    })(document.createElement('div')) : function (str) {
    ...
    function out_str(str) {
        var n_font, o_font = gene.curfont, c_font = o_font; output += str.replace(/<|>|&[^&]*?;|&|  |\$./g, function (c) {
            switch (c) {
                case '<': return "&lt;"
                case '>': return "&gt;"
                case '&': return "&amp;"
    ...
    function xy_str(x, y, str, action, w, wh) {
        ...
        switch (action) {
            case 'c': x -= wh[0] / 2; output += '" text-anchor="middle">'
                break
    ...
    function trim_title(title, is_subtitle) {
        ...
        if (cfmt.titletrim) {
            i = title.lastIndexOf(", ")
            if (i < 0 || title[i + 2] < 'A' || title[i + 2] > 'Z') { i = 0 } else if (cfmt.titletrim == true) {
        ...
        switch (action) {
            default: switch (action) {
                ...
                while (1) {
                    i = text.indexOf('\n', j)
                    if (i < 0) {
                        str = text.slice(j); wh = strwh(str); gene.curfont = font; vskip(wh[1] * cfmt.lineskipfac); xy_str(x, 0, str, action, null, wh); font = gene.curfont
                        break
                    }
                    if (i == j) {
                        vskip(parskip); p_flush(); use_font(gene.curfont)
                        while (text[i + 1] == '\n') { vskip(lineskip); i++ }
                        if (i == text.length)
                            break
                        p_start()
                    } else { str = text.slice(j, i); wh = strwh(str); gene.curfont = font; vskip(wh[1] * cfmt.lineskipfac); xy_str(x, 0, str, action, null, wh); font = gene.curfont }
                    j = i + 1
                }
    ...
    function put_words(words) {
        var p, i, j, n, nw, i2, i_end, have_text; function put_wline(p, x, right) {
            var i = 0, j, k
        ...
        for (i = 0; i < i_end || i2 < nw; i++) {
            if (i < i_end && words[i].length == 0) { blk_out(); use_font(gene.curfont) }
            vskip(cfmt.lineskipfac * gene.curfont.size)
            if (i < i_end)
                put_wline(words[i], 45., 0)
            if (i2 < nw) {
                if (put_wline(words[i2], 20. + middle, 1)) { if (--n == 0) { if (i < i_end) { n++ } else if (i2 < words.length - 1) { middle *= .6 } } }
                i2++
            }
        }
    }
    ...
    function write_headform(lwidth) {
        var c, font, font_name, align, x, y, sz, info_val = {}, info_font = clone(info_font_init), info_sz = { A: cfmt.infospace, C: cfmt.composerspace, O: cfmt.composerspace, R: cfmt.infospace }, info_nb = {}
        ...
        while (1) {
            ...
            while (1) {
                ...
                } else if (str) { xy_str(x, -y, str, align) }
    ...
    function write_heading() {
        ...
        if (composer || origin || cfmt.infoline) {
            var xcomp, align; set_font("composer"); vskip(cfmt.composerspace)
            ...
            if ((rhythm || area) && cfmt.infoline) { set_font("info"); vskip(gene.curfont.size + cfmt.infospace); put_inf2r(lwidth, 0, rhythm, area, 'r'); down1 += gene.curfont.size + cfmt.infospace }
        } else { down2 = cfmt.composerspace }
...
\n.sW{stroke:currentColor;fill:none;stroke-width:.7}', font_style = '', posx = cfmt.leftmargin / cfmt.scale, posy = 0, img = { width: cfmt.pagewidth, lm: cfmt.leftmargin, rm: cfmt.rightmargin }, defined_glyph = {}, defs = '', fulldefs = '', stv_g = { scale: 1, dy: 0, st: -1, v: -1, g: 0 }, block = {}
    ...
    function set_g() {
        if (stv_g.started) { stv_g.started = false; output += "</g>\n" }
...
1.4 -4.8 -2.4 -5.4 -3.2 -5.2\n', x, y); y -= 3.5
                ...
                } else { while (--nflags >= 0) { out_XYAB('MX Yl3 1.5 0 2 -3 -1.5z\n', x, y); y -= 3 } }
            }
        } else {
            if (!straight) {
                if (!grace) {
...
1.4 4.8 -2.4 5.4 -3.2 5.2\n', x, y); y += 3.5
    ...
    function out_tubrn(x, y, dx, dy, up, str) {
        var sw = str.length * 10, h = up ? -3 : 3; out_XYAB('<text style="font:italic 12px serif"\n\
x="X" y="Y" text-anchor="middle">A</text>\n', x + dx / 2, y + dy / 2, str); dx /= stv_g.scale
    ...
    function out_cresc(x, y, val, defl) {
        x += val; val = -val; out_XYAB('<path class="stroke"\n\
d="mX YlA ', x, y + 5, val)
        if (defl.nost)
...
<tspan dy="-4" style="font-size:10px">mb</tspan></text>\n', x - 10, y); x += 7; val -= 7
    ...
    function out_glisq(x2, y2, de) {
        var de1 = de.start, x1 = de1.x, y1 = de1.y + staff_tb[de1.st].y, ar = Math.atan2(y1 - y2, x2 - x1), a = ar / Math.PI * 180, len = (x2 - x1) / Math.cos(ar); g_open(x1, y1, a); x1 = de1.s.dots ? 13 + de1.s.xmx : 8; len = (len - x1 - 6) / 6 | 0
        if (len < 1)
            len = 1
...
src:'+ musicfont + '}'
    ...
    function mrest_expand(s) {
        ...
        while (--nb > 0) {
            s2 = sym_add(p_voice, C.BAR); s2.bar_type = "|"; s2 = sym_add(p_voice, C.REST); if (s.invis)
    ...
    function sort_all() {
        var s, s2, p_voice, v, time, w, wmin, ir, multi, prev, nb, ir2, v2, fl, new_sy, nv = voice_tb.length, vtb = [], vn = [], mrest_time = -1, sy = cur_sy
        for (v = 0; v < nv; v++)
        ...
        while (1) {
            if (new_sy && fl) {
                sy = new_sy; new_sy = null; multi = -1; vn = []
                for (v = 0; v < nv; v++) {
                    if (!sy.voices[v])
                        continue
                    ir = sy.voices[v].range
            ...
            if (time == mrest_time) {
                ...
                for (ir = 0; ir < nv; ir++) {
                    ...
                    if (nb == 0) { nb = s.nmes } else if (nb != s.nmes) {
                        mrest_time = -1
                        break
                    }
                }
            ...
            for (ir = 0; ir < nv; ir++) {
                ...
                if (!s || s.time != time || w_tb[s.type] != wmin)
                    continue
                if (s.type == C.STAVES) {
                    new_sy = s.sy; for (ir2 = 0; ir2 < nv; ir2++) {
                        if (vn[ir2] == undefined)
                            break
                    }
                    for (v2 = 0; v2 < nv; v2++) {
    ...
    function voice_adj(sys_chg) {
        ...
        s = glovar.tempo
        if (s && staves_found <= 0) {
            v = par_sy.top_voice; p_voice = voice_tb[v]; if (p_voice.sym && p_voice.sym.type != C.TEMPO) {
                s = clone(s); s.v = v; s.p_v = p_voice; s.st = p_voice.st; s.time = 0; s.next = p_voice.sym
                if (s.next)
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            if (!sys_chg && p_voice.sls.length) {
                while (1) {
                    ...
                    if (s2) {
                        if (!s.sls)
                            s.sls = []
                        s.sls.push({ ty: sl.ty, note: { s: s2 } })
                    } else { syntax(1, "Lack of ending slur(s)") }
                }
            }
            if (p_voice.ignore)
            ...
            for (; s; s = s.next) {
                switch (s.type) {
                    case C.GRACE: if (s.next && s.next.type == C.BAR)
                        s.time--
                        if (!cfmt.graceword)
                            continue
                        for (s2 = s.next; s2; s2 = s2.next) {
    ...
    Abc.prototype.set_bar_num = function () {
        var s, s2, tim, bar_time, bar_num, rep_dtime, v = cur_sy.top_voice, wmeasure = voice_tb[v].meter.wmeasure, bar_rep = gene.nbar
        ...
        if (s.time == 0) {
            for (s2 = s.ts_next; s2; s2 = s2.ts_next) {
                if (s2.type == C.BAR && s2.time) {
                    if (s2.time < bar_time) { s = s2; bar_time = s.time + wmeasure }
                    break
                }
            }
        }
            ...
            switch (s.type) {
                ...
                case C.MREST: bar_num += s.nmes - 1
                    while (s.ts_next && s.ts_next.type != C.BAR)
                        s = s.ts_next
                    break
                case C.BAR: if (s.time < bar_time) {
                    if (s.text && s.text[0] == '1') { bar_rep = bar_num; rep_dtime = bar_time - s.time }
                    break
                }
                    tim = s.time; s2 = s
                    do {
                        ...
                        if (s2.type == C.BAR && s2.text)
                            break
                        s2 = s2.next
                    } while (s2 && s2.time == tim); if (s.bar_num)
                        bar_num = s.bar_num
                    else
                        bar_num++
                    if (s2 && s2.type == C.BAR && s2.text) {
                        if (s2.text[0] == '1') { rep_dtime = 0; bar_rep = bar_num } else {
                            if (!cfmt.contbarnb)
                                bar_num = bar_rep
                            if (rep_dtime) {
                                if (cfmt.contbarnb)
    ...
    function get_map(text) {
        if (!text)
            return
        var i, note, notes, map, tmp, ns, ty, a = info_split(text)
        if (a.length < 3) {
        ...
        if (ns[0] == '*' || ns.indexOf("all") == 0) { ns = 'all' } else {
            ...
            if (ty == 'k') { ns = 'k' + ntb.indexOf(ns) } else {
                tmp = new scanBuf; tmp.buffer = ns
        ...
        i = 2
        if (a[2].indexOf('=') < 0) {
            if (a[2][0] != '*') { tmp = new scanBuf; tmp.buffer = a[2]; map[1] = parse_acc_pit(tmp) }
            if (!a[3])
                return
            i++
            if (a[3].indexOf('=') < 0) {
    ...
    function set_transp() {
        ...
        if (transp == undefined) {
            if (sndtran == undefined)
                return
        } else { curvoice.vtransp = transp }
        if (is_voice_sig()) { curvoice.key = s = clone(curvoice.okey) } else {
            s = curvoice.last_sym
            while (1) {
                if (s.type == C.KEY)
                    break
    ...
    function note_transp(s, sk, note) {
        var ak, an, d, b40, n = note.pit, a = note.acc
        if (!a && sk.k_a_acc)
            a = sk.k_map[(n + 19) % 7]
        ...
        if (d && a != an) {
            n = note.micro_n
            switch (an) {
                case 3: if (n > d / 2) { n -= d / 2; note.micro_n = n; an = a } else { an = -a }
                    break
    ...
    Abc.prototype.do_pscom = function (text) {
        ...
        switch (cmd) {
            ...
            case "map": get_map(param)
                return
            case "maxsysstaffsep": if (parse.state == 3) {
            ...
            case "multicol": generate(1)
                switch (param) {
                    case "start": blk_out(); multicol = { posy: posy, maxy: posy, lmarg: cfmt.leftmargin, rmarg: cfmt.rightmargin, state: parse.state }
                        break
                    case "new": if (!multicol) {
            ...
            case "staves": case "score": if (parse.state == 0)
                ...
                return
            case "sysstaffsep": if (parse.state == 3) {
                par_sy.voices[curvoice.v].sep = get_unit(param)
                return
            }
                break
            case "text": if (parse.state >= 2) {
                if (parse.state == 2)
                    goto_tune()
            ...
            case "transpose": if (cfmt.sound)
                ...
                for (s = curvoice.last_sym; s; s = s.prev) {
                    switch (s.type) {
                        case C.NOTE: s = clone(curvoice.okey); s.k_old_sf = curvoice.ckey.k_sf; sym_link(s)
                            break
                        case C.KEY: break
                        default: continue
                    }
            ...
            case "user": set_user(param)
                return
            case "voicecolor": if (parse.state != 3) {
                if (parse.state != 2)
                    return
                goto_tune()
            }
    ...
    Abc.prototype.do_begin_end = function (type, opt, text) {
        ...
        switch (type) {
            case "js": js_inject(text)
                break
            case "ml": if (parse.state == 3) { s = new_block(type); s.text = text } else {
                svg_flush(); if (user.img_out)
                    user.img_out(text)
            }
                break
            case "svg": j = 0
    ...
    function get_staves(cmd, parm) {
        ...
        for (i = 0; i < a_vf.length; i++) {
            ...
            if (flags & FL_VOICE) { p_voice.floating = true; p_voice.second = true } else {
                st++; if (!par_sy.staves[st]) { par_sy.staves[st] = { stafflines: '|||||', staffscale: 1 } }
    ...
    function get_vover(type) {
        ...
        if (!p_voice2) {
            p_voice2 = clone_voice(curvoice.id + 'o'); curvoice.voice_down = p_voice2; p_voice2.time = 0; p_voice2.second = true; v2 = p_voice2.v; par_sy.voices[v2] = { st: curvoice.st, second: true }
            ...
            if (f_clone) {
                p_voice3 = clone_voice(p_voice2.id + 'c'); p_voice3.second = true; v3 = p_voice3.v; par_sy.voices[v3] = { second: true, range: range + 2 }
        ...
        if (!vover) {
            vover = { bar: true, p_voice: curvoice }
            time = p_voice2.time
    ...
    function get_key(parm) {
        var v, p_voice, s, transp, sndtran, a = new_key(parm), s_key = a[0]; a = a[1]
        switch (parse.state) {
            case 1: if (s_key.k_sf == undefined && !s_key.k_a_acc) { s_key.k_sf = 0; s_key.k_none = true }
        ...
        if (s && s.type == C.METER) {
            ...
            if (!curvoice.last_sym)
                curvoice.sym = null; sym_link(s_key); s_key.next = s; s.prev = s_key; curvoice.last_sym = s
        } else { sym_link(s_key) }
    }
    ...
    function do_cloning(vs) {
        var i, eol, file = parse.file, start = parse.eol + 1, bol = start
        while (1) {
            eol = file.indexOf('\n', bol)
            if (eol < 0) {
                eol = 0
    ...
    function goto_tune(is_K) {
        var v, p_voice, s = { type: C.STAVES, dur: 0, sy: par_sy }
        set_page(); write_heading(); reset_gen(); if (glovar.new_nbar) {
    ...
    function get_sym(p, cont) {
        ...
        if (cont) {
            ...
            if (curvoice.sym_restart) { curvoice.sym_start = s = curvoice.sym_restart; curvoice.sym_restart = null } else { s = curvoice.sym_start }
            if (!s)
                s = curvoice.sym
            if (!s) {
        ...
        while (1) {
            ...
            switch (c) {
                ...
                case '!': case '"': j = ++i
                    i = p.indexOf(c, j)
                    if (i < 0) {
                        syntax(1, c == '!' ? "No end of decoration" : "No end of guitar chord"); i = p.length
                        continue
                    }
                    d = p.slice(j - 1, i + 1)
                    break
    ...
    function ly_width(s, wlw) {
        var ly, sz, swfac, align, xx, w, i, j, k, shift, p, a_ly = s.a_ly; align = 0
        for (i = 0; i < a_ly.length; i++) {
            ...
            if (s.type == C.GRACE) { shift = s.wl } else if ((p[0] >= '0' && p[0] <= '9' && p.length > 2) || p[1] == ':' || p[0] == '(' || p[0] == ')') {
                if (p[0] == '(') { sz = cwid('(') * swfac } else {
                    j = p.indexOf('Â '); set_font(ly.font)
                    if (j > 0)
            ...
            for (k = s.next; k; k = k.next) {
                switch (k.type) {
                    case C.NOTE: case C.REST: if (!k.a_ly || !k.a_ly[i])
                        xx -= 9
                    else if (k.a_ly[i].t == "-\n" || k.a_ly[i].t == "_\n")
                        xx -= shift
                    else
    ...
    function draw_lyric_line(p_voice, j, y) {
        ...
        for (; s; s = s.next) {
            ...
            if (user.anno_start || user.anno_stop) {
                s2 = { st: s.st, istart: ly.istart, iend: ly.iend, x: x0, y: y, ymn: y, ymx: y + gene.curfont.size, wl: 0, wr: w }
    ...
    function gch_tr1(p, transp) {
        ...
        for (i = 0; i < csa.length; i++) {
            p = csa[i]; o = p.search(/[ABCDEFG]/); if (o < 0)
                continue
            ip = o + 1
            a = 0
            while (p[ip] == '#') { a++; ip++ }
    ...
    Abc.prototype.gch_build = function (s) {
        var gch, wh, xspc, ix, pos = curvoice.pos.gch == C.SL_BELOW ? -1 : 1, y_above = 0, y_below = 0, y_left = 0, y_right = 0, GCHPRE = .4; s.a_gch = a_gch; a_gch = null
        if (curvoice.vtransp)
            gch_transp(s)
        for (ix = 0; ix < s.a_gch.length; ix++) {
            gch = s.a_gch[ix]
            if (gch.type == 'g') {
                if (cfmt.chordnames) {
                    gch.otext = gch.text; gch.text = gch.text.replace(/A|B|C|D|E|F|G/g, function (c) { return cfmt.chordnames[c] })
    ...
    Abc.prototype.draw_gchord = function (s, gchy_min, gchy_max) {
        var gch, text, ix, x, y, y2, hbox, h, y_above, y_below, w = 0, yav = s.dur ? (((s.notes[s.nhd].pit + s.notes[0].pit) >> 1) - 18) * 3 : 12
        for (ix = 0; ix < s.a_gch.length; ix++) {
            gch = s.a_gch[ix]
            if (gch.wh[0] > w)
                w = gch.wh[0]
        ...
        if (y_above < gchy_max)
            y_above = gchy_max
        if (y_below > gchy_min)
            y_below = gchy_min; set_dscale(s.st); for (ix = 0; ix < s.a_gch.length; ix++) {
                gch = s.a_gch[ix]; use_font(gch.font); set_font(gch.font); h = gch.font.size; hbox = gch.font.box ? 2 : 0; w = gch.wh[0]; x = s.x + gch.x; text = gch.text
                switch (gch.type) {
                    case '_': y = gch.y + y_below; y_set(s.st, 0, x, w, y - hbox)
                        break
    ...
    function set_hooks() {
        ...
        if (hook_init) { if (h.length) { set_hs(h); gh.push.apply(gh, h); abc2svg.modules.hooks = [] } } else {
            if (h.length) { gh.push.apply(gh, h); abc2svg.modules.hooks = [] }
...
abc2svg.modules = {
    ambitus: { fn: 'ambitus-1.js' }, beginps: { fn: 'psvg-1.js' }, break: { fn: 'break-1.js' }, capo: { fn: 'capo-1.js' }, clip: { fn: 'clip-1.js' }, clairnote: { fn: 'clair-1.js' }, voicecombine: { fn: 'combine-1.js' }, diagram: { fn: 'diag-1.js' }, equalbars: { fn: 'equalbars-1.js' }, grid: { fn: 'grid-1.js' }, grid2: { fn: 'grid2-1.js' }, MIDI: { fn: 'MIDI-1.js' }, pageheight: { fn: 'page-1.js' }, percmap: { fn: 'perc-1.js' }, soloffs: { fn: 'soloffs-1.js' }, sth: { fn: 'sth-1.js' }, temperament: { fn: 'temper-1.js' }, nreq: 0, hooks: [], g_hooks: [], load: function (file, relay, errmsg) {
        function get_errmsg() {
        ...
        for (var i = 0; i < ls.length; i++) {
            ...
            if (!m || m.loaded)
                continue
            m.loaded = true
            this.nreq++; abc2svg.loadjs(m.fn, function () {
                if (--abc2svg.modules.nreq == 0)
                    abc2svg.modules.cbf()
            }, function () {
                abc2svg.modules.errmsg('error loading ' + m.fn); if (--abc2svg.modules.nreq == 0)
                    abc2svg.modules.cbf()
            })
        }
...
abc2svg.version = "1.20.4-00f9c0518d"; abc2svg.vdate = "2019-10-24"